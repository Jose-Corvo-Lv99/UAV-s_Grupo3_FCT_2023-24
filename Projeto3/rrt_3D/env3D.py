...
def getballs():
    spheres = [[1.1,2.1,2,0.1],[-1.1,-2.1,2,0.1]]
    Obstacles = []
    for i in spheres:
        Obstacles.append([j for j in i])
    return np.array(Obstacles)
...
class env():
    def __init__(self, xmin=-1.1, ymin=-2.1, zmin=0, xmax=1.1, ymax=2.2, zmax=2, resolution=1):
        #xmin=0, ymin=0, zmin=0, xmax=20, ymax=20, zmax=5, resolution=1):
    # def __init__(self, xmin=-5, ymin=0, zmin=-5, xmax=10, ymax=5, zmax=10, resolution=1):  
        ...
        self.balls = getballs()
        self.OBB = np.array([obb([0,0,0],[0,0,0],R_matrix(135,0,0)),
                             obb([0,0,0],[0,0,0],R_matrix(45,0,0))])
        self.start = np.array([0, -1.1, 1.2])
