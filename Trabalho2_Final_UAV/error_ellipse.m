...
%    NOTES: C must be positive definite for this function to work properly.
    ...
    if r==3 && c==3
        ...
        Y(:) = scale*(k*XYZ(:,2)+y0);
        Z(:) = scale*(k*XYZ(:,3)+z0);
        if isempty(prop.linecolor)
            h4=surf(X,Y,Z,'FaceColor','none');
        else
            h4=surf(X,Y,Z,'FaceColor',prop.linecolor);
        end
%         colormap gray
%         alpha(0.3)
%         camlight
    ...
    elseif r==2 && c==2
        ...
        if isempty(prop.linecolor)
            ...
            h1=plot(scale*(x0+k*x),scale*(y0+k*y),prop.style,'Color',prop.linecolor);
        end
        set(h1,'zdata',z+1)
        if nargout
            h=h1;
