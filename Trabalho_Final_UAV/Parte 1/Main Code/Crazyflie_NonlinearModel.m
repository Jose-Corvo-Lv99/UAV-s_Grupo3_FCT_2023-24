...
%initial states
x(:,1) = [0; 0; 0; pi/180; pi/180; 0; 0; 0; 0; 0; 0; 0]; % In reality the drone iself should estimate its initial state or at least position (by communicating with some reference station (it could be a controller I think)) when it starts

%equilibrium inputs
T1_eq2=0.08580928036602421844178304220226;%Definition of the thrust
T2_eq2=T1_eq2;
T3_eq2=T1_eq2;
T4_eq2=T1_eq2;

%----------
v_eq_inercial=[11.5342; -9.8824;0]
p_eq=v_eq_inercial*(t>=0)+x(1:3,1)*ones(1,N);
v_eq=[11.5324; -9.8774;0.3737]*(t>=0);
lbd_eq=[pi/180;pi/180;0]*(t>=0);
om_eq=[0;0;0]*(t>=0);
...
Gl_Hov_p_110=[Gl_Hov(4,2), Gl_Hov(5,3);
              Gl_Hov(6,4), Gl_Hov(3,1);
              Gl_Hov(2,2), Gl_Hov(1,3)]
      
Gl_Hori_p_110=[Gl_Hori(4,2), Gl_Hori(5,3);
               Gl_Hori(6,4), Gl_Hori(3,1);
               Gl_Hori(2,2), Gl_Hori(1,3)]
